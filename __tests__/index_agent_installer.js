...
test('install_options_easy', async () => {
    const event = createEvent({
        event: 'test event', properties: {
            ...
            "install_options": "--dont-start-it --dont-wait --claim-token 3gciBd6HYGp7Z2v2fJDd6meraFoUT4QpVYcHTE253KujJPStNQhXi9cicTgEEc_mNiQNxAYtHlZNpC1a2NQz57fV6aZaa2vPvyPYw9hsv_SOfzfWxMdQ6L-PPOyM9e9N2HAVp7E --claim-rooms 22ff1e07-8e9c-41ad-b141-5bd95fbf95d1 --claim-url https://app.netdata.cloud"
        }
    })
    const eventCopy = await processEvent(clone(event), getMeta())
    expect(eventCopy['properties']['event_source']).toEqual("agent installer")
